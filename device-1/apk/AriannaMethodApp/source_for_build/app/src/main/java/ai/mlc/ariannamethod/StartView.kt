...
fun StartView(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(text = "Arianna Method", color = MaterialTheme.colorScheme.onPrimary) },
    ...
    { paddingValues ->
        Column(
            ...
            // BETA: Show "Start Chat" button when no models (API-only mode)
            if (appViewModel.modelList.isEmpty()) {
                Column(
                    modifier = Modifier
                        .fillMaxSize()
                        .padding(top = 40.dp),
